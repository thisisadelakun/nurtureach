...
const BitcoinUI = () => {
  ...
  useEffect(() => {
    const fetchSubmissions = async () => {
      try {
        const submissionsRef = firestore.collection('bitcoin');
        ...
        console.error('Error fetching bitcoin submissions:', error);
      }
    };
  ...
  const handleDelete = async () => {
    try {
      ...
      await firestore.collection('bitcoin').doc(submissionId).delete();

      await storage.ref().child(`bitcoinPictures/${submissionId}_receipt.jpg`).delete();
      ...
      console.log('Bitcoin submission deleted successfully.');
    } catch (error) {
      console.error('Error deleting bitcoin submission:', error);
    }
  };

  return (
    <div style={{ width: '100%', margin: '5rem auto', minHeight: '100vh' }}>
      <h1>Bitcoin Submissions</h1>
      <div>
      ...
      <Modal show={deleteSuccess} onHide={() => setDeleteSuccess(false)}>
        ...
        <Modal.Body>Bitcoin submission deleted successfully.</Modal.Body>
        <Modal.Footer>
